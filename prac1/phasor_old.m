...
% Square axis is critical to get correspondence between phasor and time signal y-axes
  ...
  zoom on;
% Add the axis lines
  line([-MagScale MagScale],[0 0],'Color',[.5 .5 .5],'EraseMode','none');
  line([0 0],[-MagScale MagScale],'Color',[.5 .5 .5],'EraseMode','none');
...
% Also square - see above
  ...
  zoom on;
% Add axis lines
  line(tlimit,[0 0],'Color',[.5 .5 .5],'EraseMode','none');
  line([0 0],[-MagScale MagScale],'Color',[.5 .5 .5],'EraseMode','none');
...
% Draw phasors
  ...
  phasorlines(1) = line(xr(:),yr(:),'Color','r','LineWidth',2);
  if (numphasors > 1) 
    phasorlines(2) = line(xg(:),yg(:),'Color','g','LineWidth',2);
  else
    phasorlines(2) = line(NaN,NaN,'Color','g','LineWidth',2);
  end;
...
% Collect data for next phase  
  ...
  %fud = [hrot, hsig, tlimit(2), phasorlines, hrotline, hsigline];
  fud = struct('hrot', hrot, 'hsig', hsig, 'tlimit', tlimit(2), 'phasorlines', phasorlines, 'hrotline', hrotline, 'hsigline', hsigline);

  set(PHASOR_WINDOW_HANDLE,'UserData',fud);
% Debugging
%  disp(['original phasor data = ']);
%  disp(num2str(fud));
...
% Time instants that will be drawn
  instants = linspace(0,tlimit,Sample); % instants = 0:Sample:tlimit;
...
% Update complex trajectory
    xrot = get(hrotline,'XData');
    yrot = get(hrotline,'YData');
    set(hrotline,'XData',[xrot xtip],'YData',[yrot ytip]);
  
% Update real-valued signal
    xsig = get(hsigline,'XData');
    ysig = get(hsigline,'YData');
    set(hsigline,'XData',[xsig instants(n)],'YData',[ysig ytip]);

% Update phasor lines
    set(phasorlines(1),'XData',xr(:),'YData',yr(:));
    if (numphasors > 1) 
      set(phasorlines(2),'XData',xg(:),'YData',yg(:));
    end;
...
% Save data
  set(PHASOR_WINDOW_HANDLE,'UserData',[hrot, hsig, tlimit, phasorlines, hrotline, hsigline]);
end;
